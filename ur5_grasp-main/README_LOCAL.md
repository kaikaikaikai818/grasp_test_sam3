...
菜单 `6` 通过后，菜单 `7` 单独验证无接触下降。按 `p` 到观察点，按 `y` 完成
高位方向对齐，等待画面显示 `SAFE DESCENT READY 3/3` 后按一次小写 `d`。程序
先在安全高度对准手柄，再低速下降，并停在D435i测得的手柄顶面上方 40 mm。
这一阶段不接触工具、不初始化夹爪，并且同一次运行只允许执行一次。实际夹持前
仍须完成 `grasp_surface_calibration.json` 的一次性夹爪高度标定。检查40毫米间隙
后按小写 `u`，程序保持当前XY和姿态，只垂直回升到至少250毫米，再按 `q` 退出。
如果旧版本已退出并把机械臂留在低位，可更新代码后使用菜单 `8`，进入后按一次
小写 `u` 执行相同的纯垂直安全回升。
